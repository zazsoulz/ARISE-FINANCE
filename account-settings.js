(function(root){
  ...
  function status(text,error=false){
    const el=document.getElementById("canonicalAccountStatus");
    if(!el) return;
    el.textContent=text||"";
    el.className="tiny "+(error?"negative":"muted");
  }
  ...
  async function saveAccount(){
    ...
    status("Сохраняю…");
    try{
      if(remote&&remote.currentSession()){
        await remote.updateAccount({name,notifications_enabled:notifications});
      }
      state.account.name=name;
      state.account.notifications=notifications;
      ...
      status("Не удалось сохранить аккаунт.",true);
    }
  }
  ...
  async function uploadAvatar(file){
    ...
    try{
      const displayUrl=await remote.uploadAvatar(file);
      state.account.avatar=displayUrl;
      delete state.account.password;
      saveState();
      status("Фото обновлено.");
      render();
    }catch(error){
      console.error("ARISE avatar upload",error);
      status(error.message||"Не удалось загрузить фото.",true);
    }
  }
  ...
  function showPasswordChange(){
    ...
    document.getElementById("canonicalPasswordSave").onclick=async()=>{
      const password=document.getElementById("canonicalNewPassword").value;
      const line=document.getElementById("canonicalPasswordStatus");
      ...
      try{
        ...
        line.textContent="Не удалось изменить пароль.";line.className="tiny negative";
      }
    };
  }
  ...
  async function logout(){
    ...
    try{
      if(root.ARISE_SUPABASE) await root.ARISE_SUPABASE.signOut();
    }catch(error){console.error("ARISE logout",error);}
    if(root.ARISE_LOCAL_ACCOUNTS) root.ARISE_LOCAL_ACCOUNTS.deactivate();
    else{state.account.registered=false;delete state.account.password;saveState();}
    activePage="home";
    renderAuth();
  }
  ...
  root.renderSettings=function(){
    ...
    const grid=page&&page.querySelector(".grid");
    if(!grid) return;

    for(const kicker of grid.querySelectorAll(".kicker")){
      if((kicker.textContent||"").trim()==="АККАУНТ"){
        const owner=kicker.closest(".c12,.c8,.c7,.c6,.c5,.c4,.c3");
