...
test("home uses a continuous material flow without route rails",()=>{
  ...
  assert.match(v3Source,/const HOME_FLOW_TEXTURE_ASPECT=720\/1279/);
  assert.match(v3Source,/uniform sampler2D uFlowTexture/);
  assert.match(v3Source,/function startReferenceHomeFlow\(canvas,image,reducedMotion\)/);
  assert.match(v3Source,/gl\.texImage2D\(gl\.TEXTURE_2D,0,gl\.RGBA,gl\.RGBA,gl\.UNSIGNED_BYTE,image\)/);
  assert.match(v3Source,/vec2 primaryUv=mix\(uv\+bodyWarp,poolUv,poolGate\)/);
  assert.match(v3Source,/vec2 layerAUv=primaryUv/);
  assert.match(v3Source,/vec2 layerBUv=primaryUv/);
  assert.match(v3Source,/vec2 layerCUv=primaryUv/);
  assert.match(v3Source,/mat2\(cs,-sn,sn,cs\)\*poolPoint/);
  assert.match(v3Source,/float travelling=/);
  assert.match(v3Source,/gl\.drawArrays\(gl\.TRIANGLE_STRIP,0,4\)/);
  assert.match(v3Source,/canvas\.dataset\.flowRenderer="reference-advection-webgl"/);
  assert.doesNotMatch(v3Source,/gl\.drawArrays\(gl\.LINES/);
  assert.doesNotMatch(v3Source,/gl\.drawArrays\(gl\.POINTS/);
  assert.doesNotMatch(v3Source,/startWebGLHomeFlow/);
