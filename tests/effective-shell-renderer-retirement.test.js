...
test('production loader retires legacy topbar from the effective compatibility shell',()=>{
  ...
  assert.match(index,/retireLegacyRenderer\(html,"renderTopbar",`\/\* ={57}\n   NAV\n={57} \*\/`\)/,'renderTopbar is not retired before canonical runtime boot');
...
test('renderer retirement helper fails closed when a compatibility boundary drifts',()=>{
  ...
  assert.equal(shell.indexOf('function renderTopbar(){')<shell.indexOf(navMarker),true,'topbar/nav boundary order drifted');
});
