...
function boot(){
  ...
  const ctx={
    ...
    renderNav:()=>'<nav class="nav"></nav>',
    money:value=>`${Math.round(Number(value)||0)} ₽`,
    formatMonth:key=>key,
    ...
    ARISE_ANALYTICS:{
      ...
      series:()=>[
        {month:'2026-07',income:100000,expenses:40000},
        {month:'2026-08',income:150000,expenses:50000}
      ],
...
test('analytics month selector is local and does not mutate global app month',()=>{
  const {ctx,dom}=boot();
  ctx.renderAnalytics();
  const select=dom.window.document.getElementById('analyticsMonth');
  select.value='2026-07';
  select.onchange();
  assert.equal(ctx.activeMonth,'2026-08');
  assert.equal(ctx.ARISE_ANALYTICS_UI.getSelectedMonth(),'2026-07');
  assert.match(dom.window.document.querySelector('.analytics-head').textContent,/2026-07/);
});
