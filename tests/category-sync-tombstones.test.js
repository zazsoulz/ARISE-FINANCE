...
function bootLocalStore(){
  ...
  const profile={id:'local-profile',name:'Основной',categories:[category],goals:[],transactions:[],settings:{currency:'RUB'},ariseSync:{remoteId:'22222222-2222-4222-8222-222222222222',dirty:false}};
...
test('sync engine deletes tombstoned remote categories and clears the tombstone only after success',async()=>{
  ...
  const context={
    ...
    state:{profiles:[]},
    ARISE_SUPABASE:{
      getClient:()=>({from:table=>{assert.equal(table,'finance_categories');return query;}}),
      currentSession:()=>null
    }
  };
  context.globalThis=context;
