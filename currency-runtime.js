(function(root){
  ...
  function planSnapshot(profile,originalAmount,originalCurrency){
  ...
  root.calculateIncomePlan=function(profile,income,date){
    ...
    if(snap.conversionPending){
      return {
        valid:false,fxPending:true,total:0,allocations:[],goalAllocations:[],reserve:0,remainder:0,distributed:0,
        date,originalAmount:snap.originalAmount,originalCurrency:snap.originalCurrency,baseCurrency:snap.baseCurrency,fxSnapshot:snap
      };
    }
    ...
    if(plan&&plan.fxPending){
      ...
      container.innerHTML=`
        <div class="notice warning">
          Для распределения ${fx.format(plan.originalAmount,plan.originalCurrency)} нужен курс ${plan.originalCurrency} → ${plan.baseCurrency}.
          ARISE не будет придумывать курс и искажать твой финансовый план.
        </div>
        <div class="actions"><button class="btn primary" id="refreshFxForIncome">Обновить курс</button></div>`;
      ...
      if(button) button.onclick=async()=>{
        ...
        try{
          ...
          console.error("ARISE FX income refresh",error);
          toast("Не удалось обновить курс. Попробуй позже.");
    ...
    if(container&&plan&&plan.originalCurrency&&plan.originalCurrency!==base){
      ...
      container.insertAdjacentHTML("afterbegin",`
        <div class="notice" style="margin-bottom:14px">
          ${fx.format(plan.originalAmount,plan.originalCurrency)} ≈ <strong>${fx.format(plan.total,base)}</strong>
          · курс ${Number(snap.exchangeRateToBase||0).toLocaleString("ru-RU",{maximumFractionDigits:4})}
          · ${escapeHTML(snap.fxSource||"курс")}
        </div>`);
    }
  };

  root.createIncomeTransaction=function(profile,data){
    const plan=root.currentIncomePlan||((typeof currentIncomePlan!=="undefined")?currentIncomePlan:null);
    const snap=plan&&plan.fxSnapshot&&!plan.fxSnapshot.conversionPending
      ? plan.fxSnapshot
    ...
    const tx=originalCreateIncome(profile,{
      ...data,
      amount:snap.baseAmount,
      currency:snap.baseCurrency
    });
    tx.originalAmount=snap.originalAmount;
    tx.originalCurrency=snap.originalCurrency;
    tx.baseAmount=snap.baseAmount;
    tx.baseCurrency=snap.baseCurrency;
    tx.exchangeRateToBase=snap.exchangeRateToBase;
    tx.fxSource=snap.fxSource;
    tx.fxFetchedAt=snap.fxFetchedAt;
    tx.conversionPending=false;
    return tx;
  };
  ...
  root.createExpenseTransaction=function(profile,data){
    ...
    if(snap.conversionPending){
      throw new Error("Для расхода в другой валюте сначала нужен актуальный или сохранённый курс.");
    }
    const tx=originalCreateExpense(profile,{
      ...data,
      amount:snap.baseAmount,
      currency:snap.baseCurrency
    });
  ...
  function originalDisplay(tx){
    if(!tx) return {amount:0,currency:"RUB"};
    return {
  ...
  function currentBaseValue(tx,profile){
    return fx.transactionBaseAmount(tx,baseCurrency(profile),currentBook(),{preferCurrentRate:true});
  }

  async function refreshRates(force=false){
    const result=await fx.ensureRateBook({force});
    root.dispatchEvent&&root.dispatchEvent(new CustomEvent("arise:fx",{detail:result}));
    return result;
  }

  root.ARISE_CURRENCY_RUNTIME={baseCurrency,currentBook,planSnapshot,originalDisplay,currentBaseValue,refreshRates};

  if(root.addEventListener){
    root.addEventListener("online",()=>refreshRates(false).catch(()=>{}));
    setTimeout(()=>refreshRates(false).catch(()=>{}),0);
  }
