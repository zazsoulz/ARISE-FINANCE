...
comment on column public.finance_transactions.amount is
  'Original transaction amount in currency.';
comment on column public.finance_transactions.currency is
  'Original transaction currency; never rewritten by later FX changes.';
comment on column public.finance_transactions.base_currency is
  'Financial profile base currency at transaction conversion time.';
comment on column public.finance_transactions.exchange_rate_to_base is
  'Snapshot multiplier: original amount * rate = base_amount.';
comment on column public.finance_transactions.base_amount is
  'Transaction amount normalized to base_currency using the stored snapshot rate.';
comment on column public.finance_transactions.fx_source is
  'Source identifier for the exchange-rate snapshot.';
comment on column public.finance_transactions.fx_fetched_at is
  'Timestamp of the rate snapshot used for conversion.';
