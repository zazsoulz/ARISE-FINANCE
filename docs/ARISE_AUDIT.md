...
Basis: `docs/ARISE_SPEC.md` vs `main` at `54b9d60dc9f9ddec7fe8b6ab98289953114f774c` (2026-08-23).
...
The highest-value remaining work is now operational beta verification, final shell consolidation and cross-screen product polish. Physical compatibility-shell removal has progressed materially: legacy `renderTopbar`, navigation source, `renderHome`, `renderIncome`, `renderGoals` and `renderHistory` are physically gone from `app-shell.html`; legacy `renderAnalytics` is the next guarded removal target. `renderSettings` remains physically present only as compatibility source even though canonical Settings ownership is already external. Vercel remains intentionally outside the active development loop until stable beta/production.
...
- ✅ Legacy `renderTopbar`, `NAV_ITEMS + renderNav`, `renderHome`, `renderIncome`, `renderGoals` and `renderHistory + historyMonthBlock` have been physically removed from `app-shell.html`.
...
- ✅ A guarded physical-removal transform exists for legacy `renderAnalytics()` and is wired into CI.
- 🟡 Legacy `renderAnalytics()` source is still physically present and is the next source-removal target.
- 🟡 Legacy `renderSettings()` source is still physically present even though canonical Settings rendering is owned by `settings-ui.js`; remove it only after an equivalent guarded transform and full regression pass.
...
1. Physically remove legacy `renderAnalytics()` using the existing guarded transform, update the retirement registry and run the full CI gate.
2. Prepare and then physically remove legacy `renderSettings()` with the same fail-closed staged approach.
3. Continue shrinking remaining compatibility helpers/CSS only after ownership is explicit and regression protected.
4. Review loading/error/offline/retry states screen by screen for consistent actions and copy.
5. Finish real-device chart/touch interaction polish and final icon pass.
6. Run standalone mobile/desktop browser verification using the canonical manifest-based preview artifact.
7. Perform the final dead-primary-action inventory and accessibility pass after consolidation.
