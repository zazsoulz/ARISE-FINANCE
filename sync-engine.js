(function(root){
  ...
  function conflictError(entity,label){const conflict=entity&&entity[META_KEY]&&entity[META_KEY].conflict;if(!conflict)return null;const error=new Error(`${label||"Данные"} изменены на другом устройстве после последней синхронизации. Синхронизация остановлена, чтобы не потерять изменения.`);error.code="ARISE_SYNC_CONFLICT";error.conflict=conflict;return error;}
  ...
  async function getUser(){const s=session();if(s&&s.user)return s.user;const c=client();if(!c)return null;const {data,error}=await c.auth.getUser();if(error)throw error;return data.user||null;}
  ...
  function migrateEntityTombstones(profile,{metaKey,entity}){
    ...
    if(!outbox||typeof outbox.enqueue!=="function")throw new Error("ARISE sync outbox is required to migrate legacy tombstones.");
    for(const remoteIdValue of ids){
      outbox.enqueue(profile,{entity,entityLocalId:null,entityRemoteId:remoteIdValue,action:"delete"});
    }
    delete meta[metaKey];
    return ids.length;
  }
  ...
  function seedEntityOutbox(profile,entity,collection){
    const outbox=root.ARISE_SYNC_OUTBOX;if(!outbox||!outbox.enqueue||!outbox.list)return 0;
    const queued=new Set(outbox.list(profile,entity).map(item=>String(item.entityLocalId)));
    let count=0;
    for(const item of profile[collection]||[]){if(remoteId(item)||queued.has(String(item.id)))continue;outbox.enqueue(profile,{entity,entityLocalId:item.id,action:"upsert"});queued.add(String(item.id));count++;}
    return count;
  }
  ...
  async function drainEntityOutbox(profile,profileId,user){
    ...
    return {drained,seededCategories,seededGoals,legacyDeletes:legacyCategoryDeletes+legacyGoalDeletes,legacyMigrated:legacyCategoryDeletes+legacyGoalDeletes};
  }
  ...
  async function findExistingTransaction(c,userId,tx){
    ...
    if(isUuid(tx.id)){const {data,error}=await c.from("finance_transactions").select("id").eq("user_id",userId).eq("client_mutation_id",tx.id).maybeSingle();if(error)throw error;if(data&&data.id)return data.id;}
    const {data,error}=await c.from("finance_transactions").select("id,payload").eq("user_id",userId).contains("payload",{localId:tx.id}).limit(1);if(error)throw error;return data&&data[0]&&data[0].id||null;
  }
  ...
  function transactionCurrencySnapshot(profile,tx){
    ...
    return {originalAmount,originalCurrency,baseCurrency:base,baseAmount,exchangeRateToBase:Number.isFinite(rate)&&rate>0?rate:(originalCurrency===base?1:null),fxSource:tx.fxSource||tx.fx_source||(originalCurrency===base?"identity":null),fxFetchedAt:tx.fxFetchedAt||tx.fx_fetched_at||null};
  }
  ...
  async function syncTransaction(profile,profileId,user,tx){
    ...
    const payload={profile_id:profileId,user_id:user.id,type:transactionType(tx),amount:fx.originalAmount,currency:fx.originalCurrency,base_currency:fx.baseCurrency,exchange_rate_to_base:fx.exchangeRateToBase,base_amount:fx.baseAmount,fx_source:fx.fxSource,fx_fetched_at:fx.fxFetchedAt,date:tx.date||new Date().toISOString().slice(0,10),source:String(tx.source||""),note:String(tx.note||""),category_id:category?remoteId(category):null,goal_id:goal?remoteId(goal):null,funding_source:tx.fundingSource||null,controlled_amount:Math.max(0,Number(tx.controlledAmount)||0),uncontrolled_amount:Math.max(0,Number(tx.uncontrolledAmount)||0),remainder:Math.max(0,Number(tx.remainder)||0),reserve_amount:Math.max(0,Number(tx.reserve)||0),client_mutation_id:isUuid(tx.id)?tx.id:null,payload:{localId:tx.id,month:tx.month||null,ledgerAmount:fx.baseAmount,allocations:tx.allocations||[],goalAllocations:tx.goalAllocations||[],fundingBreakdown:tx.fundingBreakdown||null}};
    let id=await findExistingTransaction(c,user.id,tx);let data,error;
    if(id)({data,error}=await c.from("finance_transactions").update(payload).eq("id",id).select("id").single());else({data,error}=await c.from("finance_transactions").insert(payload).select("id").single());
  ...
  function seedTransactionOutbox(profile){
    const outbox=root.ARISE_SYNC_OUTBOX;if(!outbox||!outbox.enqueue||!outbox.list)return 0;
    const queued=new Set(outbox.list(profile,"transaction").map(item=>String(item.entityLocalId)));let count=0;
  ...
  async function flushTransactionOutbox(profile,profileId,user){
    const outbox=root.ARISE_SYNC_OUTBOX;if(!outbox||!outbox.list)return 0;
    const c=client();let count=0;
    for(const mutation of outbox.list(profile,"transaction")){
      try{
        if(mutation.action==="delete"){
          let id=mutation.entityRemoteId||null;if(!id&&mutation.entityLocalId)id=await findExistingTransaction(c,user.id,{id:mutation.entityLocalId});
          if(id){const {error}=await c.from("finance_transactions").delete().eq("id",id).eq("profile_id",profileId);if(error)throw error;}
        }else{
          const tx=(profile.transactions||[]).find(item=>String(item.id)===String(mutation.entityLocalId));if(tx)await syncTransaction(profile,profileId,user,tx);
        }
  ...
  async function syncProfile(profile,profileId,user){
    ...
    const c=client();const {error}=await c.from("finance_profiles").update({name:profile.name||"Профиль",base_currency:currency(profile),settings:{...(profile.settings||{})}}).eq("id",profileId);if(error)throw error;
    ...
    return {txCount:transactionOutboxCount,outboxCount:transactionOutboxCount+entityResult.drained,seededTransactions,seededEntities:entityResult.seededCategories+entityResult.seededGoals,legacyDeletes:entityResult.legacyDeletes,legacyMigrated:entityResult.legacyMigrated};
  }

  function publish(result){lastResult=result;if(root.dispatchEvent)root.dispatchEvent(new CustomEvent("arise:sync",{detail:lastResult}));return lastResult;}
  ...
  async function pushAll(){
    ...
    running=true;const startedAt=new Date().toISOString();publish({status:"busy",startedAt});
    try{
      const serverProfiles=await loadServerProfiles();let txCount=0,outboxCount=0,seededTransactions=0,seededEntities=0,legacyDeletes=0,legacyMigrated=0;
      for(let i=0;i<(state.profiles||[]).length;i++){
        ...
        txCount+=result.txCount;outboxCount+=result.outboxCount;seededTransactions+=result.seededTransactions;seededEntities+=result.seededEntities;legacyDeletes+=result.legacyDeletes;legacyMigrated+=result.legacyMigrated||0;
      }
      ...
      return publish({status:"synced",profiles:(state.profiles||[]).length,transactions:txCount,outboxMutations:outboxCount,seededTransactions,seededEntities,legacyDeletes,legacyMigrated,startedAt,finishedAt:new Date().toISOString()});
    }catch(error){
      console.error("ARISE sync",error);const status=error&&error.code==="ARISE_SYNC_CONFLICT"?"conflict":"error";publish({status,message:error.message||"Sync failed",conflict:error&&error.conflict||null,startedAt,finishedAt:new Date().toISOString()});if(status==="conflict")return lastResult;throw error;
  ...
  root.ARISE_SYNC={pushAll,schedule,markDirty,remoteId,conflictError,assertNoConflict,transactionCurrencySnapshot,applyCategoryTombstones,applyGoalTombstones,migrateEntityTombstones,seedEntityOutbox,drainEntityOutbox,seedTransactionOutbox,flushTransactionOutbox,lastResult:()=>lastResult};
