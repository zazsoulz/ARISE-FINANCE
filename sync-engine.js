(function(root){
  ...
  async function ensureProfile(localProfile,user,serverProfiles,index){
    ...
    if(error) throw error;
    id=data.id;mark(localProfile,id);return id;
  }
  ...
  async function syncCategories(profile,profileId,user){
    ...
    for(const category of profile.categories||[]){
      ...
      const id=remoteId(category);let data,error;
      ...
      if(error) throw error;mark(category,data.id);count++;
    }
    return count;
  }
